...
#
# `# syntax=docker/dockerfile:1` は指定しない。この指定は BuildKit にビルドのたび
# 外部フロントエンドイメージを取得させる。当該タグは可変であり固定にならないうえ、
# 取得失敗がビルド全体の失敗となる (実際に CI で発生した)。本ファイルは外部
# フロントエンド固有の機能を使用しないため、組み込みフロントエンドで足りる。
#
# 方針:
