...
# shell 実装を読み込み、git が入っている経路と NixOS-WSL で一時的に gitMinimal を
# 使う経路から共通に実行する。対象 distro は Windows のファイルシステムから隔離されて
# いるため、スクリプトのパスではなく内容を標準の sh -c へ渡す。
function Initialize-Repository {
  $repositoryScriptPath = Join-Path $PSScriptRoot 'wsl-repository.sh'
  if (-not (Test-Path -LiteralPath $repositoryScriptPath -PathType Leaf)) {
    throw "$repositoryScriptPath が見つかりません。dotfiles の checkout を確認してください。"
  }
  $repositoryCommand = [System.IO.File]::ReadAllText(
    $repositoryScriptPath,
    [System.Text.Encoding]::UTF8
  )
  if ([string]::IsNullOrWhiteSpace($repositoryCommand)) {
    throw "$repositoryScriptPath が空です。"
  }
  $repositoryCommand = $repositoryCommand.Replace("`r`n", "`n")
