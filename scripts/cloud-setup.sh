...
# 記録する環境変数。
#
# 名前を列挙する方式とする。env の全体を取って名前のパターン (*TOKEN* 等) で除外する
# 方式は採らない。パターンに一致しない名前の秘密があれば、その値が記録されるためである。
# ここに足すのは、値が秘密になりえないものに限る。
readonly LOGGED_ENV=(
  CLAUDE_CODE_REMOTE
  CLAUDE_ENV_FILE
  CODEX_HOME
...
# 構成全体が中断される。log_finish で起きた場合は、成功した構成の終了状態を 1 に変えてしまう。
#
...
# 指定された nixpkgs のパッケージを store へ入れ、profile に追加する。
#
...
install_toolchain() {
  ...
  #
  # ただし探す順は逆にし、$HOME に依存しない方を先に採る。ここで作成する symlink は
